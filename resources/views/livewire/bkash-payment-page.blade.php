        ...
        <div class="p-6 sm:p-10">
            <div class="space-y-6">
                ...
                <form wire:submit.prevent="completePayment" class="space-y-4">
                    <div>
                        <label for="payment_phone" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">bKash Number (From which you sent)</label>
                        <div class="relative">
                            ...
                            <input wire:model="payment_phone" type="phone" id="payment_phone" class="block w-full pl-10 pr-3 py-3 border border-gray-300 dark:border-slate-700 rounded-xl leading-5 bg-white dark:bg-slate-800 text-gray-900 dark:text-white placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-pink-500 focus:border-pink-500 sm:text-sm transition-all duration-200" placeholder="01XXXXXXXXX">
                        </div>
                        @error('payment_phone') <p class="mt-1 text-xs text-red-500">{{ $message }}</p> @enderror
                    </div>

                    <div>
                        <label for="payment_amount" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">bKash Number (From which you sent)</label>
                        <div class="relative">
                            <div class="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                                <span class="text-gray-500">৳</span>
                            </div>
                            <input wire:model="payment_amount" type="number" id="payment_amount" class="block w-full pl-10 pr-3 py-3 border border-gray-300 dark:border-slate-700 rounded-xl leading-5 bg-white dark:bg-slate-800 text-gray-900 dark:text-white placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-pink-500 focus:border-pink-500 sm:text-sm transition-all duration-200" placeholder="{{ $order->grand_total }}">
                        </div>
                        @error('payment_amount') <p class="mt-1 text-xs text-red-500">{{ $message }}</p> @enderror
                    </div>
                    ...
                    <div>
                        ...
                        <div class="relative">
                            ...
                            <input wire:model="transaction_id" type="text" id="transaction_id" class="block w-full pl-10 pr-3 py-3 border border-gray-300 dark:border-slate-700 rounded-xl leading-5 bg-white dark:bg-slate-800 text-gray-900 dark:text-white placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-pink-500 focus:border-pink-500 sm:text-sm transition-all duration-200" placeholder="e.g. 8N7A6D5C">
                        </div>
                        @error('transaction_id') <p class="mt-1 text-xs text-red-500">{{ $message }}</p> @enderror
                    </div>
